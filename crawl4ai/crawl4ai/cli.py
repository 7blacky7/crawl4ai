...
# ════════════════════════════════════════════════════════════════════
# OAuth Command Group - Claude CLI OAuth Management
# ════════════════════════════════════════════════════════════════════
...
@cli.group("oauth")
def oauth_cmd():
    """Manage Claude OAuth authentication

    Commands for setting up Claude CLI OAuth for use with Crawl4AI:
    - status - Check if Claude CLI is installed and logged in
    - setup  - Interactive guide to set up Claude OAuth
    """
    pass
...
@oauth_cmd.command("status")
def oauth_status_cmd():
    """Show Claude OAuth status

    Checks if Claude CLI is installed and whether you're logged in.
    Shows how to use Claude OAuth with Crawl4AI once authenticated.
    """
    console.print(Panel("[bold cyan]Claude OAuth Status[/bold cyan]", border_style="cyan"))

    status = get_claude_auth_status()

    # CLI Installation Status
    if status["cli_installed"]:
        console.print("[green][OK][/green] Claude CLI installed")
    else:
        console.print("[red][X][/red] Claude CLI not installed")
    ...
    # Auth Status
    if status["logged_in"]:
        console.print("[green][OK][/green] OAuth active - Logged in!")
        if status["user"]:
            console.print(f"     User: [cyan]{status['user']}[/cyan]")
    else:
    ...
    # Usage Instructions
    console.print("\n[bold]Usage in Crawl4AI:[/bold]")
    console.print("  [cyan]LLMConfig(provider=\"claude-oauth\")[/cyan]")
    console.print("\n[dim]This uses your Claude CLI authentication instead of API keys.[/dim]")


@oauth_cmd.command("setup")
def oauth_setup_cmd():
    """Interactive Claude OAuth setup guide

    Walks you through the process of setting up Claude CLI OAuth:
    1. Check if Claude CLI is installed
    2. Guide through authentication
    3. Verify the setup works
    """
    console.print(Panel(
        "[bold cyan]Claude OAuth Setup Guide[/bold cyan]\n"
        "This will help you set up Claude CLI OAuth for use with Crawl4AI.",
        border_style="cyan"
    ))

    # Step 1: Check CLI installation
    console.print("\n[bold]Step 1: Check Claude CLI Installation[/bold]")

    if check_claude_cli_installed():
        console.print("[green][OK][/green] Claude CLI is installed")
    else:
        console.print("[red][X][/red] Claude CLI is not installed")
        console.print("\n[yellow]To install Claude CLI:[/yellow]")
        console.print("  [cyan]npm install -g @anthropic-ai/claude-code[/cyan]")
        console.print("\nAfter installation, run [cyan]crwl oauth setup[/cyan] again.")
        return

    # Step 2: Check current auth status
    console.print("\n[bold]Step 2: Check Authentication Status[/bold]")

    status = get_claude_auth_status()

    if status["logged_in"]:
        console.print("[green][OK][/green] You're already logged in!")
        if status["user"]:
            console.print(f"     User: [cyan]{status['user']}[/cyan]")
    else:
        console.print("[yellow][!][/yellow] Not logged in yet")
        console.print("\n[yellow]To authenticate:[/yellow]")
        console.print("  [cyan]claude auth login[/cyan]")
        console.print("\nThis will open a browser window to complete OAuth authentication.")

        # Ask if user wants to run the command now
        if Confirm.ask("\n[cyan]Would you like to run 'claude auth login' now?[/cyan]"):
            console.print("\n[dim]Running 'claude auth login'...[/dim]\n")
            try:
                subprocess.run(["claude", "auth", "login"], check=False)

                # Re-check status
                console.print("\n[bold]Verifying authentication...[/bold]")
                new_status = get_claude_auth_status()

                if new_status["logged_in"]:
                    console.print("[green][OK][/green] Authentication successful!")
                else:
                    console.print("[yellow][!][/yellow] Authentication not completed")
                    console.print("     Try running [cyan]claude auth login[/cyan] manually")
                    return
            except Exception as e:
                console.print(f"[red]Error:[/red] {str(e)}")
                return
        else:
            console.print("\nRun [cyan]claude auth login[/cyan] when ready, then [cyan]crwl oauth status[/cyan] to verify.")
            return

    # Step 3: Show usage
    console.print("\n[bold]Step 3: Using Claude OAuth in Crawl4AI[/bold]")
    console.print("\n[green]Setup complete![/green] You can now use Claude OAuth in your code:\n")

    code_example = '''from crawl4ai import LLMConfig, LLMExtractionStrategy

# Use Claude OAuth instead of API key
llm_config = LLMConfig(provider="claude-oauth")
...
strategy = LLMExtractionStrategy(
    ...
    console.print(Panel(code_example, title="Python Example", border_style="green"))

    console.print("\n[bold]CLI Usage:[/bold]")
    console.print("  [cyan]crwl https://example.com -j \"Extract all links\"[/cyan]")
    console.print("  [dim](Will use Claude OAuth if configured as default provider)[/dim]")

    console.print("\n[bold]Set as Default:[/bold]")
    console.print("  [cyan]crwl config set DEFAULT_LLM_PROVIDER claude-oauth[/cyan]")
